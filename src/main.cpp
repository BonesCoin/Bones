...
// Copyright (c) 2013-2014 Bones Developers
...
uint256 hashGenesisBlock("0xfd58a028eae6c6ec1e74c534bdb65787f25cd290c79b4ea891331fb406e6ab70");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bones: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bones Signed Message:\n";
...
{
    // Bones: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    int rand = generateMTRandom(seed, 3);
    int rand1 = 0;
    int rand2 = 0;
    int rand3 = 0;
    int rand4 = 0;
	...
	if(nHeight < 51)
	{
	    nSubsidy = (9 + rand + (int64)round(GetDifficulty())) * COIN;
	}
	else if(nHeight < 61)
	{
		nSubsidy = 215513 * COIN;; // 2% premine (Difficulty 50)
	}
	else if(nHeight < 71)
    {
        nSubsidy = (8 + rand + (int64)round(GetDifficulty())) * COIN;

	printf("Difficulty is %f \n" ,GetDifficulty());
	printf("+ BonusDifficulty %d\n",(int64)round(GetDifficulty())); // 25
    }
	else if(nHeight < 200000)
    {
        nSubsidy = (7 + rand + (int64)round(GetDifficulty())) * COIN;
    ...
    else if(nHeight < 300000)
    {
        ...
        seed = hex2long(cseed);
        rand1 = generateMTRandom(seed, 3);
        nSubsidy = (5 + rand1 + (int64)round(GetDifficulty())) * COIN;
    ...
    else if(nHeight < 400000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand2 = generateMTRandom(seed, 2);
        nSubsidy = (4 + rand2 + (int64)round(GetDifficulty())) * COIN;
    ...
    else if(nHeight < 500000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand3 = generateMTRandom(seed, 2);
        nSubsidy = (3 + rand3 + (int64)round(GetDifficulty())) * COIN;
    ...
    else if(nHeight < 600000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand4 = generateMTRandom(seed, 2);
        nSubsidy = (2 + rand4 + (int64)round(GetDifficulty())) * COIN;
    ...
    else if(nHeight < 700000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand5 = generateMTRandom(seed, 2);
        nSubsidy = (1 + rand5 + (int64)round(GetDifficulty())) * COIN;
    ...
    else if(nHeight < 800000)
    {
        ...
        cseed = cseed_str.c_str();
        seed = hex2long(cseed);
        rand6 = generateMTRandom(seed, 2);
        nSubsidy = (1 + rand6 + (int64)round(GetDifficulty())) * COIN;
		
	printf("Difficulty is %f \n" ,GetDifficulty());
	printf("+ BonusDifficulty %d\n",(int64)round(GetDifficulty())); // 25	
    }
    else if(nHeight < 1000100)
    {
...
static const int64 nTargetTimespan = 6 * 60 * 60; // Bones: every 6 hours
static const int64 nTargetSpacing = 60; // Bones: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bones: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Bones: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Bone2Bone";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x377126faf4b86efd69d65520ab1f1d3109eff004fb284814aa23340b5f381e03"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BonesMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BonesMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BonesMiner(CWallet *pwallet)
{
    printf("BonesMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Bones-miner");
    ...
    try { loop {
        ...
        printf("Running BonesMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BonesMiner, pwallet));
}
