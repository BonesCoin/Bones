...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("Bones"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("Bones://"))
    {
        uri.replace(0, 11, "Bones:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bones.lnk";
}
...
{
    return GetAutostartDir() / "Bones.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bones-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Bones-qt [" + tr("command-line options") + "]                     " + "\n";
